...
class DeviceList(unittest.TestCase):
    ...
    def setUp(self):
        self.api_url = 'coding/outDevice/list'
    ...
    def test_deviceList_01(self):
        ...
        sql = "select * from ep_user_device where user_id = 2 order by id desc limit "+str(limit * (page - 1))+","+str(limit)
        print(sql)
    ...
    def test_deviceList_04(self):
        ...
        for d in re_data:
            ...
            re_device_id_list.append(re_device_id)
        # 2通过sql语句从数据库中查询期望的设备id列表：根据入参中的查询条件、页数、每页的个数组合sql
        sql = "select * from ep_user_device where user_id = 2 and CONCAT(mac,remarks,device_fingerprint) like '%" + condition + "%' order by id desc limit "+str(limit * (page - 1))+","+str(limit)
        print(sql)
        self.mysql.cursor.execute(sql)
        sql_result = self.mysql.cursor.fetchall()
        sql_device_id_list = []
