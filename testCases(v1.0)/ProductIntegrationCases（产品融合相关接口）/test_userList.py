...
@describe : 鉴权1.0 bim融合-企业用户管理-用户查询接口
...
class UserList(unittest.TestCase):
    ...
    def test_user_list_01(self):
        ...
        self.mysql.cursor.execute(
            "select login_name from ep_users where is_deleted='0' and is_disabled='" + isDisabled + "' and CONCAT(login_name,name,phone,email,department) like '%" + condition + "%'order by id desc limit " + str(
                startRow) + "," + str(
                limit))
        expect_data = []
    ...
    def test_user_list_04(self):
        ...
        startRow = (page - 1) * limit
        self.mysql.cursor.execute(
            "select login_name from ep_users where is_deleted='0' and is_disabled='%s' order by id desc limit %s,%s" % (
