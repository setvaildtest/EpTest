...
@author : zhangaihua
@Time : 2019/8/27
@File : test_userEnable.py
@describe :鉴权1.0 bim融合-企业用户管理-用户启用接口
...
class userEnable(unittest.TestCase):
    def setUp(self):
        api_url = "coding/userInfo/active"
        self.url = conf_url() + api_url
        self.mysql = MySqlUtil()
        # 通过sql创建用户用于测试用户启用
        self.test_user_id_list1, self.test_login_name_list1 = ChildUserSqlUtil.insert_users(4, '1')
        self.test_user_id_list2, self.test_login_name_list2 = ChildUserSqlUtil.insert_users(4, '0')

    def test_user_enable_01(self):
        u'批量启用禁用状态的用户，请求成功'
        print('用户启用api_url：=================>', self.url)
        data = self.test_login_name_list1
        data = jsonTostr(data)
    ...
    def test_user_enable_02(self):
        u'批量启用启用状态的用户，请求成功'
        print('用户启用api_url：=================>', self.url)
        data = self.test_login_name_list2
        data = jsonTostr(data)
        re = sceneDecide(self.url, data)
        print('用户启用接口入参：=================>\n', data)
        print('用户启用接口响应：=================>\n', re)
