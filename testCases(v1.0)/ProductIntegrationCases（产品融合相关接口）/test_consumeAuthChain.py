...
@describe :  该模块包括鉴权1.0BAM融合 - 消费认证链接口的测试用例
...
class ConsumeAuthChain(unittest.TestCase):
    ...
    def test_consumeAuthChain_02(self):
        ...
        print("re:", re)
        # 断言
        expect_message = "epSessionid 不能为空;"
        actual_message = re['message']
        self.assertEqual(expect_message, actual_message)
    ...
    def test_consumeAuthChain_03(self):
        u'当前待消费的认证方式为空，请求失败'
        # 创建认证链
        epSessionId = self.custom_auth_chain("pwd", self.login_name)
        data = {
            "appName": "OA",
            "authResult": True,
            "authType": "",
    ...
    def test_consumeAuthChain_07(self):
        ...
        # 创建认证链
        epSessionId = self.custom_auth_chain("pwd", "")
        data = {
            "appName": "OA",
            "authResult": True,
            "authType": "pwd",
